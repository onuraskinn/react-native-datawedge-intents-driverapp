  ...
  private fun toBundle(obj: JSONObject?): Bundle? {
    ...
    try {
      ...
      while (keys.hasNext()) {
        ...
        if (obj[key] is String) returnBundle.putString(key, obj.getString(key))
        else if (key.equals("keystroke_output_enabled", ignoreCase = true)) returnBundle.putString(
          key,
          obj.getString(key)
        )
          ...
          if (jsonArray[0] is String) {
            ...
            for (k in 0 until length) bundleArray[k] = toBundle(jsonArray.getJSONObject(k))
